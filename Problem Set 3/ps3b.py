...
class Patient(object):
    ...
    def update(self):
        ...
        virusesCopy1 = self.viruses[:]

        for v in virusesCopy1:
            try:
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for trials in range(numTrials):
        ...
        for timestep in range(300):
            popChanges.append(newPatient.update())
        data += popChanges
    data_avg = data/numTrials

    pylab.figure(dpi=1200)
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        if all([self.isResistantTo(drug) for drug in activeDrugs]) == True:
            ...
            if prob <= self.maxBirthProb * (1 - popDensity):
               offsprResistances = self.resistances.copy()
               
               for key in self.resistances.keys():
                   prob1 = random.random()
                   if prob1 <= self.mutProb:
                       i = offsprResistances[key]
                       offsprResistances[key] = not i
                       
               return ResistantVirus(self.maxBirthProb, self.clearProb, offsprResistances, self.mutProb)
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        for v in self.viruses:
            if all([v.isResistantTo(drug) for drug in drugResist]) == True:
                resistantViruses.append(v)
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for trials in range(numTrials):
        ...
        popChanges, popChangesDrug = [], []
        
        for timestep in range(150):
            newPatient.update()
            popChanges.append(newPatient.getTotalPop())
            popChangesDrug.append(newPatient.getResistPop(['guttagonol']))
        ...
        data += popChanges
        data1 += popChangesDrug
    
    data_avg = data/numTrials
    data1_avg = data1/numTrials
    
    pylab.figure(dpi=1200)
    pylab.plot(list([float('{0:.1f}'.format(i)) for i in data_avg]), label="Non-resistant population")
    pylab.plot(list([float('{0:.1f}'.format(j)) for j in data1_avg]), label="Guttagonol Resistant population")
